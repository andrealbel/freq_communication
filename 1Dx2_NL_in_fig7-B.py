...
#  print(T-1000./f[k])
  ...
  min2 = sorted(min22)
  if len(max1)>0:
     phase1[k] = ((max1[0][1]-maxf[1])*pi*f[k]/500.)#% (2*pi)
  if len(max2)>0:
     phase2[k] = ((max2[0][1]-maxf[1])*pi*f[k]/500.)#% (2*pi)
...
pyplot.plot(f,Zi1,'--',c= 'blue', label=r'$ V^{0}_{max,1}$')
for i in range(len(puntosM)):
   ccual2 = puntosM[i][0]
   pyplot.scatter(f[ccual2],KM[ccual2],s=70,linewidth=1.5,c='white', ec='black',zorder=3,alpha=1)
pyplot.xlabel(r'$f$ [Hz]',fontsize=font1)
#pyplot.ylabel(r'$\epsilon$',fontsize=26)
...
pyplot.plot(f,Pi1,'--',c= 'blue', label=r'$ \Phi^{0}_{ntwk,1}$')
pyplot.xlabel(r'$f$ [Hz]',fontsize=font1)
pyplot.xticks([0,50,100,150,200],fontsize=font1)
